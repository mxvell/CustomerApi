...
public class ApiExceptionHandler {
    ...
    public ResponseEntity<Object> handleUnderAgeUserException(UnderAgeUserException exception){
       ...
       ApiException apiException = new ApiException(exception.getMessage(), internalServer,ZonedDateTime.now(ZoneId.of("Z"))
       );
       return new ResponseEntity<>(apiException,internalServer);
    }

    @ExceptionHandler(value = {DeleteUserWithIdException.class})
    public ResponseEntity<Object> handlerDeletedExcellent(DeleteUserWithIdException exception){
            HttpStatus ok = HttpStatus.OK;
            ApiException apiException = new ApiException(exception.getMessage(),ok,ZonedDateTime.now(ZoneId.of("Z"))
            );
